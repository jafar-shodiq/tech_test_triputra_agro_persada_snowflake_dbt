...
2. Activate your virtual environment and install the required dbt dependencies:
    ...
    ```
3. Initialize dbt from the terminal:
    ```bash
    dbt init
    ```
4. Execute the following command to build the entire model lineage:
    ```bash
    dbt run
    ```
...
* **Accepted Values**: Validates that `order_status` only contains official statuses (e.g., `delivered`, `shipped`, `canceled`).
